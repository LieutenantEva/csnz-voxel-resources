...
API_URL = "http://52.28.231.59:3000/v5/slots/list/search"
API_ITEMS_LIMIT_PER_OFFSET = 100

S3_BUCKET_NAME = "nxe-cz-prod-studio"
S3_RESOURCE_URL = f"https://{S3_BUCKET_NAME}.s3.amazonaws.com/resources_prod/{{}}.vxl"
S3_IMAGE_URL = f"https://{S3_BUCKET_NAME}.s3.amazonaws.com/images_prod/{{}}.vmg"

OUT_RESOURCE_DIR = Path("./resources_prod/")
OUT_IMAGE_DIR = Path("./images_prod/")
...
ASYNC_CONCURRENCY = 8
ASYNC_RETRIES = 3
ASYNC_CHUNK_SIZE = 1024 * 1024 # 1MB
...
def calculate_md5 (path: Path):
    ...
    with path.open("rb") as file:
        while chunk := file.read(ASYNC_CHUNK_SIZE):
            md5.update(chunk)
...
async def fetch_page (session, offset):
    params = {
        "limit": API_ITEMS_LIMIT_PER_OFFSET,
        "offset": offset,
    }
    async with session.get(API_URL, params=params) as response:
        ...
        if not data.get("succeed"):
            raise RuntimeError(f"API failed at offset={offset}")
...
async def collect_ids ():
    ...
    async with aiohttp.ClientSession() as session:
        offset = 0
        while True:
            print(f"[API] offset={offset}")
            slots = await fetch_page(session, offset)
            if not slots:
                break
            ...
            print(f"  resources={len(resource_ids)} images={len(image_ids)}")
            offset += 1
    return (resource_ids, image_ids)

# ============================================================
# Phase 2: Async downloader
# ============================================================

async def download_file (session, semaphore, url, output_path: Path):
    headers = {}
    if output_path.exists() and not ALLOW_OVERWRITE:
        local_md5 = calculate_md5(output_path)
        headers["If-None-Match"] = f'"{local_md5}"'
        print(f"[CHECK] {output_path.name} md5={local_md5}")
            ...
            try:
                async with session.get(url, headers=headers) as response:
                    ...
                    with temp_path.open("wb") as file:
                        async for chunk in response.content.iter_chunked(ASYNC_CHUNK_SIZE):
                            file.write(chunk)
                ...
                print(f"[ERROR] {output_path.name}: {error}")
                if temp_path.exists():
                    temp_path.unlink()
...
async def download_all (resource_ids, image_ids):
    jobs = []
    for resource_id in resource_ids:
        jobs.append((
            S3_RESOURCE_URL.format(resource_id),
            OUT_RESOURCE_DIR / f"{resource_id}.vxl",
        ))
    for image_id in image_ids:
        jobs.append((
            S3_IMAGE_URL.format(image_id),
            OUT_IMAGE_DIR / f"{image_id}.vmg",
        ))

    print()
    print("==============================")
    print(f"Files queued: {len(jobs)}")
    print(f"Concurrency: {ASYNC_CONCURRENCY}")
    print(f"Overwrite: {ALLOW_OVERWRITE}")
    print("==============================")
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
        results = await asyncio.gather(*(
            download_file(session, semaphore, url, path)
            for url, path
            in jobs
        ))
...
async def main ():
    ...
    resource_ids, image_ids = (await collect_ids())

    print()
    print("==============================")
    print("API crawl complete")
    print(f"Unique resources: {len(resource_ids)}")
    print(f"Unique images:    {len(image_ids)}")
    print("==============================")

    await download_all(resource_ids, image_ids)
